...
function parse_day2(input :: String) :: AbstractArray{Password}
	line_regexp = r"^(\d+)-(\d+) ([a-z]): ([a-z]+)$"
	result = Array{Password, 1}()
	for line ∈ split(input, '\n')
		m = match(line_regexp, line)
		push!(
			result,
			Password(
				UnitRange(parse(Int64, m.captures[1]), parse(Int64, m.captures[2])),
				m.captures[3][1],
				m.captures[4]
			)
		)
...
function nb_of_valid_passwords_part1(passwords :: AbstractArray{Password})
	sum(
		function (password)
			m = 0
			for c in password.p
...
function nb_of_valid_passwords_part2(passwords :: AbstractArray{Password})
	sum(
		password ->
...
# ╔═╡ 2b112350-347a-11eb-1725-5f70e3623379
input_day2_raw = "1-13 r: gqdrspndrpsrjfjx
...
input_day2 = parse_day2(input_day2_raw)
